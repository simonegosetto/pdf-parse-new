...
class SmartPDFParser {
	 ...
	 * CPU-aware: thresholds adapt to available cores
	...
	selectMethod(analysis, userOptions) {
		...
		// Baseline CPU from training data (most common in benchmarks)
		const BASELINE_CPU = 24;

		/**
		 * Normalize page threshold based on available CPU cores
		 * More cores → handle more pages with parallelism
		 * Fewer cores → use parallelism sooner
		 */
		const cpuNormalizedThreshold = (baselinePages) => {
			return Math.floor(baselinePages * (cpuCores / BASELINE_CPU));
		};
		...
		// Huge PDFs (CPU-normalized threshold) - Best: multi-processing/threading for parallelism
		//
		// Benchmark data shows both methods are viable:
		//   Historical (9417 samples): Workers 2355ms, Processes 2775ms
		//   Recent test (9924 pages):  Processes 4468ms, Workers 6963ms
		//   Both methods: 2-4x faster than batch (11723ms+)
		//
		// Decision: Use processes by default (more consistent, better isolation)
		//           but workers are available as alternative via forceMethod
		//
		// CPU Normalization Examples (baseline: 24 cores = 1000 pages):
		//   4 cores  → 167 pages  (use parallelism sooner on low-core CPUs)
		//  12 cores  → 500 pages
		//  24 cores  → 1000 pages (baseline from training data)
		//  48 cores  → 2000 pages (defer parallelism on high-core CPUs)
		const parallelThreshold = cpuNormalizedThreshold(1000);

		if (pages > parallelThreshold) {
			// Default to processes (more consistent results in production)
			// Users can override with forceMethod: 'workers' if they prefer
			const optimalWorkers = this.calculateOptimalWorkers(analysis);

			console.log(`[SmartPDFParser] Using ${optimalWorkers} processes (${cpuCores} cores × ${this.options.oversaturationFactor} oversaturation)`);
			...
			return {
				...
				config: {
					chunkSize: 500,
					batchSize: 10,
					maxProcesses: optimalWorkers
				},
				parser: PDFProcesses
			};
		}

		// Fallback to batch-50 for PDFs below workers threshold
		return {
			name: 'batch',
			config: {
				parallelizePages: true,
